...
namespace TestWithValue.Web.Controllers
{
    public class TaskController : Controller
    {
        ...
        public TaskController(ITaskService taskService, ITicketService ticketService, /*IConverter converter,*/ ILogger<TaskController> logger, ICaseService caseService)
        {
        ...
        [HttpGet]
        public async Task<IActionResult> GetOccupiedTimes(string date)
        {
            ...
            {
                // تبدیل تاریخ شمسی به میلادی
                var parts = date.Split('/'); // فرض فرمت YYYY/MM/DD
                if (parts.Length != 3)
                    throw new FormatException("فرمت تاریخ صحیح نیست.");
            ...
            // دریافت وظایف مرتبط با تاریخ مشخص‌شده
            var tasks = await _taskService.GetTasksByDateAsync(DateOnly.FromDateTime(taskDateMiladi)); // منتظر دریافت داده‌ها

            var occupiedTimes = tasks
                .Where(task => task.TaskStartTime.HasValue && task.TaskEndTime.HasValue)
                .Select(task => new
                {
                    StartTime = task.TaskStartTime?.Hour ?? 0, // ساعت شروع
                    EndTime = task.TaskEndTime?.Hour ?? 0      // ساعت پایان
                })
                .ToList();
        ...
        // متد برای دریافت لیست پرونده‌ها
        [HttpGet]
        public async Task<IActionResult> GetCases()
        {
            var cases = await _caseService.GetAllCasesAsync();

            // استفاده از Count() به جای Count (مقداردهی صحیح به تعداد عناصر لیست)
            if (cases == null || !cases.Any()) // استفاده از Any() برای چک کردن خالی بودن لیست
            {
                return NotFound("هیچ پرونده‌ای موجود نیست.");
            }
        ...
        //[HttpPost]
        //public async Task<IActionResult> AcceptCase(int caseId, DateOnly taskDate)
        //{
        //    // پیدا کردن پرونده مورد نظر
        //    var caseItem = await _context.Tbl_Cases.FirstOrDefaultAsync(c => c.CaseId == caseId);
        //    if (caseItem == null)
        //    {
        //        return NotFound("پرونده مورد نظر پیدا نشد.");
        //    }

        //    // ساخت یک وظیفه جدید برای این پرونده
        //    var task = new Tbl_Task
        //    {
        //        Title = caseItem.Title,
        //        TaskDate = taskDate,
        //        IsDone = false,
        //        CaseId = caseItem.CaseId, // ارتباط با پرونده
        //        UserId = User.FindFirstValue(ClaimTypes.NameIdentifier) // شناسه کاربر فعلی
        //    };

        //    // اضافه کردن وظیفه جدید به جدول
        //    _context.Tbl_Tasks.Add(task);
        //    await _context.SaveChangesAsync();

        //    return Ok("پرونده با موفقیت پذیرفته شد.");
        //}
